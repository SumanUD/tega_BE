...
class Productv2Controller extends Controller
{
    public function index(){
        ...
        return view('Admin.addproductv2',['blogs'=>$blogs]);
    }
    ...
    public function editproduct_function(Request $request){
        ...
        $what_offer = [];
        foreach ($request->input('variation_offer_description_old') as $index => $description) { 
            $what_offer[] = [
                'index' => 'what_offer'.$index,
                'description' => $description,
                'gallery_path' => "image"
        ...
        $key_benefit = [];
        foreach ($request->input('variation_key_benefits_question_old') as $index => $question) { 
            $key_benefit[] = [
                'index' => 'key_benefit'.$index,
                'question' => $question,
                'answer' => $request->input('variation_key_benefits_answer_old')[$index]
            ]; 
        }
        ...
        $product_feature = [];
        foreach ($request->input('old_pr_fea_title') as $index => $title) { 
            $product_feature[] = [
                'index' => 'product_feature'.$index,
                'title' => $title,
                'description' => $request->input('old_pr_fea_desc')[$index],
                'gallery_path' => "image",
            ]; 
        }
        
        $product_pdf = [];
        foreach ($request->input('old_pdf_title') as $index => $pdf_title) { 
            $product_pdf[] = [
                'index' => 'product_pdf'.$index,
                'pdf_title' => $pdf_title,
                'pdf_path' => "Pdf",
        ...
        if ($request->has('product_image')) {
            ...
            $product_imageName = $products['product_image'];
        }
